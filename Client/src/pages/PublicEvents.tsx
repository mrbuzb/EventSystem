...
export const PublicEvents: React.FC = () => {
  ...
  const loadEvents = async () => {
    try {
      setLoading(true);
      const data = await apiService.getAllPublicEvents();
      setEvents(data);
    } catch (err: any) {
  ...
  const handleSubscribe = async (eventId: number) => {
    try {
      ...
      // Refresh events to update subscription status
      await loadEvents();
    } catch (err: any) {
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Discover and join public events happening in your community
          </p>
        </div>
      </div>
        ...
        {filteredEvents.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm ? 'Try adjusting your search terms.' : 'No public events are currently available.'}
            </p>
          </div>
        ) : (
